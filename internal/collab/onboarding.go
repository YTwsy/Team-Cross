...
func (a *App) onboarding(w http.ResponseWriter, r *http.Request, path string) bool {
	...
	case path == "invitations/preview" && r.Method == "POST":
		...
		respond(w, map[string]any{"title": inv.Title, "host": inv.Host, "expiresAt": inv.ExpiresAt, "transport": inv.Transport, "runtimeMode": inv.RuntimeMode, "readOnly": inv.ReadOnly}, e)
		return true
