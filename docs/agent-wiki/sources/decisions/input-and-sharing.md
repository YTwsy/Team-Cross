...
邀请只绑定一个协作，使用临时 TLS 证书、SPKI 指纹和 secret，默认一小时有效。B 的本机 Core 管理远端连接与凭据，本机客户端通过本机代理进入共享运行时。

当前实现只包含 LAN，不沿用旧原型的 LAN → Tailnet → Tailcat 自动降级。过期、撤销和协议不匹配不是可忽略的网络重试信号。
