...
| 邀请 | 绑定单一协作的临时 LAN 访问凭据；不是永久会话包或离线交接包 |
...
`state` 描述协作准备、就绪或失败，加入记录还可表示已离开、结束和到期；`online` 描述运行时连接；`busy` 描述轮次运行；`sharing` 描述远端访问是否开放；`connected` 描述直接客户端是否连接。不能用其中一个字段代替其他状态。
