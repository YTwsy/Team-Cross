...
- Web 类型检查、49 项测试和 production build 通过；10 项批注测试覆盖引用、草稿、单层回复、中文输入法快捷键、失败重试和详情信息布局。
...
- 详情布局续验确认参与协作位于右栏批注上方，共享状态属于执行信息卡，技术信息是“协作上下文”的最后一个标签；长批注正文使用独立内容层级，引用卡片本身保持不变。
- 共享详情展开前后主内容的顶部、高度和文档高度相同；技术信息标签不发送不存在的 `kind=technical` 请求。1024 和 768 CSS 像素下均无水平溢出。
- 原批注闭环用例的独立浏览器控制台无错误和警告。后续详情布局复核使用已停止 reader 的保存快照，控制台只有预期的上下文读取失败，不作为布局错误；保存失败、相同请求重试与 IME 不误提交由前端自动化回归覆盖。

最终截图和本次复核材料位于工作区 `output/playwright/annotations-2026-09-13/` 与 `output/playwright/collaboration-layout-2026-09-13/`，不属于发布包。内置浏览器截图缩放失真后已用独立 Chrome 截图替换；没有将失真截图用于布局通过结论。
