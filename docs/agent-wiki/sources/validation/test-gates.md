...
本页说明变更后应运行什么，以及每种检查能支持什么结论。指定版本的实际结果见 [2026-09-09 验收记录](native-collaboration-2026-09-09.md)，该记录不是后续提交自动通过的证明。
...
该用例建立测试仓库、来源和两个 fork，执行证明文件操作。全部真实模型调用限定 `gpt-5.6-luna`；不要把测试配置写入产品默认值。模拟模型名称只能证明转发、状态和拒绝语义，不能证明其他模型实际可用。
