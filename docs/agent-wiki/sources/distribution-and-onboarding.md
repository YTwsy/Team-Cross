...
A 选择来源和目录，点击“创建并邀请”。创建结果先持久化，分享失败保留该协作并跳转到详情重试分享，不重复 fork。启动不要求 `--repo`；此高级参数只设置本机辅助上下文。

邀请同时给出 `teamcross://join?invite=…`、原始 `tcx2.` 内容及安装指引。App 将邀请经带本机凭据的接口暂存，浏览器只携带随机 pending ID。暂存最多 32 项、10 分钟，仍受原邀请期限约束。邀请声明的名称和主机在预览显示，实际加入使用原有 TLS pin 验证；预览不联系远端、不启动模型。
