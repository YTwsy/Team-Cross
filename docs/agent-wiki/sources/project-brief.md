...
真实工作会因新发现而重新确认需求、补查历史约束或调整方案。产品关注有经验的人能否及时参与判断，不以减少人工介入次数作为研发提效的充分依据。从已有 Session 和会话记录入手，让参与者保留熟悉的工作区、原生客户端与个人 Agent，按需分享材料、加入讨论或接过输入继续。

协作空间和共享执行托管在发起者的 Mac，连接由用户选择局域网或实验性 Tailcat；参与者安装本地工具即可加入，无需额外注册 Team Cross 账号或集中部署协作平台。这里的本机托管不改变各自 Provider 的模型调用方式。个人资源库则用于找回此前分享或参与的材料、批注与执行上下文，供当前 Agent 按选择读取，不自动汇集所有私人会话。
...
当前面向普通 Git 仓库、macOS 与多人协作空间，支持三人及以上参与；Codex 为主线，Claude Code 原生 TUI 为实验性 Provider。连接方式由用户显式选择局域网或实验性 Tailcat，不自动降级或切换。不包含 submodule 支持、强制 Round、独立 Evidence 体系、离线交接包、专用 patch/PR 流程、Tailnet 控制平面集成或 Tailcat 服务可用性承诺。
...
项目原创代码及未另行声明的资产采用 [GPL-3.0-only](../../../LICENSE)，与 OpenSurge 保持一致。第三方依赖保留各自的许可证；完整协议文本由根目录 `LICENSE` 维护。
