...
只读空间与原生执行空间使用可复用空间链接和独立成员身份；`selfId` 表示本机身份，有执行时 `writer` 绑定具体成员。材料发布、阅读与讨论无需输入权。启用执行保留链接和成员，在成员列表开放新增执行访问，见 [协作空间任务页](collaboration-spaces.md)；同机三 Core 与真实多台 Mac 分别验收。
...
- 同一链接多人分别加入，链接关闭或重置不撤销已有成员。默认链接在本次共享中持续有效。断线、睡眠、关闭客户端或 B 重启不撤销资格。
