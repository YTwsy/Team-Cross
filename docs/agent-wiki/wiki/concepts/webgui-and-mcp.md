...
执行详情用 [ReadingTabs.tsx](../../../../packages/web/src/components/ReadingTabs.tsx) 在阅读面板标题栏切换材料和上下文；共用稳定高度，各自内部滚动并保留阅读状态，切换不主动滚动整页。批注定位自动切到对应标签，模式说明由标题旁的入口按需展开。只读空间没有执行上下文标签。材料正文在对应卡片内展开并占满列表宽度，卡片顶部统一显示当前版本标题、元信息和操作，不另设重复标题或收起入口；选择与收藏始终绑定所读版本，收藏有高亮、文字与失败反馈。批注提示与专注阅读并入材料工具栏或上下文类型栏。完整行为见 [上下文阅读](../../sources/product-flows.md#上下文阅读)。
