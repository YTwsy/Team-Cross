...
`read_material` 首页显式请求轮次目录；默认流完整给出用户/助手消息，长工具输出只给精确前缀、总长度和读取提示。WebGUI 的“读取完整输出”以及 Agent 的 `turnId + itemId + startOffset` 都切换到单条分页，按条游标不会溢出到其他 item；深处批注也直接定位该条，而不顺序吞下之前的日志。所有偏移继续使用 UTF-16，折叠投影不改变固定版本正文或批注位置。
